...
/*
 * Copyright 2008 NxProGA\Google Inc.
 *
...
namespace NxProGA\Google\AccessToken;

use DateTime;
use DomainException;
use Exception;
use ExpiredException;
use NxProGA\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use NxProGA\Firebase\JWT\Key;
use NxProGA\Firebase\JWT\SignatureInvalidException;
use NxProGA\Google\Auth\Cache\MemoryCacheItemPool;
use NxProGA\Google\Exception as GoogleException;
...
/**
 * Wrapper around NxProGA\Google Access Tokens which provides convenience functions
 *
 */
class Verify
{
    ...
    */
    ...
    public function verifyIdToken($idToken, $audience = null)
    {
        ...
        // set NxProGA\phpseclib constants if applicable
    ...
    {
        $jwtClass = 'JWT';
        if (class_exists('\NxProGA\Firebase\JWT\JWT')) {
            $jwtClass = 'NxProGA\Firebase\JWT\JWT';
        }
    ...
    private function getRsaClass()
    {
        ...
        if (class_exists('NxProGA\phpseclib\Crypt\RSA')) {
    ...
    private function getBigIntClass()
    {
        ...
        if (class_exists('NxProGA\phpseclib\Math\BigInteger')) {
    ...
    private function getOpenSslConstant()
    {
        ...
        if (class_exists('NxProGA\phpseclib\Crypt\RSA')) {
            return 'NxProGA\phpseclib\Crypt\RSA::MODE_OPENSSL';
        }
    ...
    /**
   * NxProGA\phpseclib calls "phpinfo" by default, which requires special
   * whitelisting in the AppEngine VM environment. This function
   * sets constants to bypass the need for NxProGA\phpseclib to check phpinfo
   *
