...
use NxProGA\Google\Auth\Credentials\InsecureCredentials;
    ...
    UpdateMetadataInterface
{
    ...
    private static function getGuzzleMajorVersion()
    {
        ...
        if (defined('NxProGA\GuzzleHttp\ClientInterface::VERSION')) {
     ...
     * @return \NxProGA\GuzzleHttp\Client
     */
    public static function makeHttpClient(
        ...
        if (self::getGuzzleMajorVersion() === 5) {
            $client = new \NxProGA\GuzzleHttp\Client($httpClientOptions);
        ...
        $stack = \NxProGA\GuzzleHttp\HandlerStack::create();
        $stack->push($middleware);

        return new \NxProGA\GuzzleHttp\Client([
            'handler' => $stack,
            'auth' => 'google_auth',
        ] + $httpClientOptions);
    }
