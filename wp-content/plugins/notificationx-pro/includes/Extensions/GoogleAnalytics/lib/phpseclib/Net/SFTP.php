 ...
 *
 *    $sftp = new \NxProGA\phpseclib\Net\SFTP('www.domain.tld');
...
class SFTP extends SSH2
{
     ...
     *
     * \NxProGA\phpseclib\Net\SSH2::exec() uses 0 and \NxProGA\phpseclib\Net\SSH2::read() / \NxProGA\phpseclib\Net\SSH2::write() use 1.
     *
    ...
    /**#@+
     * @access public
     * @see \NxProGA\phpseclib\Net\SFTP::put()
    */
    /**
     ...
     * @param int $port
     * @param int $timeout
     * @return \NxProGA\phpseclib\Net\SFTP
     * @access public
     */
    function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        // the order, in this case, matters quite a lot - see \NxProGA\phpseclib\Net\SFTP::_parseAttributes() to understand why
        ...
        // see \NxProGA\phpseclib\Net\SFTP::_parseLongname() for an explanation
    ...
    function _init_sftp_connection()
    {
        ...
        /*
         ...
         in draft-ietf-secsh-filexfer-13 would be quite impossible.  As such, what \NxProGA\phpseclib\Net\SFTP would do is close the
    ...
    function chdir($dir)
    {
        ...
        // see \NxProGA\phpseclib\Net\SFTP::nlist() for a more thorough explanation of the following
        $response = $this->_get_sftp_packet();
        switch ($this->packet_type) {
     ...
     * Returns general information about a file or symbolic link
     *
     * Determines information without calling \NxProGA\phpseclib\Net\SFTP::realpath().
    ...
    function chown($filename, $uid, $recursive = false)
    {
        /*
         ...
         NxProGA\phpseclib _could_ auto append the dns_domain to $uid BUT what if it shouldn't
         have one? NxProGA\phpseclib would have no way of knowing so rather than guess NxProGA\phpseclib
     ...
     *
     * By default, \NxProGA\phpseclib\Net\SFTP::put() does not read from the local filesystem.  $data is dumped directly into $remote_file.
     * So, for example, if you set $data to 'filename.ext' and then do \NxProGA\phpseclib\Net\SFTP::get(), you will get a file, twelve bytes
     ...
     * @return bool
     * @access public
     * @internal ASCII mode for SFTPv4/5/6 can be supported by adding a new function - \NxProGA\phpseclib\Net\SFTP::setMode().
     */
