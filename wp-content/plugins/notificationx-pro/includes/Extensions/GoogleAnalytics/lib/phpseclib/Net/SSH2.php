 ...
 *
 *    $ssh = new \NxProGA\phpseclib\Net\SSH2('www.domain.tld');
 ...
 *    include 'vendor/autoload.php';
 *
 *    $key = new \NxProGA\phpseclib\Crypt\RSA();
...
namespace NxProGA\phpseclib\Net;

use NxProGA\phpseclib\Crypt\Base;
use NxProGA\phpseclib\Crypt\Blowfish;
use NxProGA\phpseclib\Crypt\Hash;
use NxProGA\phpseclib\Crypt\Random;
use NxProGA\phpseclib\Crypt\RC4;
use NxProGA\phpseclib\Crypt\Rijndael;
use NxProGA\phpseclib\Crypt\RSA;
use NxProGA\phpseclib\Crypt\TripleDES;
use NxProGA\phpseclib\Crypt\Twofish;
use NxProGA\phpseclib\Math\BigInteger; // Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use NxProGA\phpseclib\System\SSH\Agent;
...
class SSH2
{
     ...
     *
     * @see \NxProGA\phpseclib\Net\SSH2::bitmap
     * @access private
     */
     ...
     *     the other side.
     *
     * @see \NxProGA\phpseclib\Net\SSH2::_send_channel_packet()
     * @see \NxProGA\phpseclib\Net\SSH2::_get_channel_packet()
     * @access private
    */
    ...
    /**#@+
     * @access public
     * @see \NxProGA\phpseclib\Net\SSH2::getLog()
    */
    /**
    ...
    /**#@+
     * @access public
     * @see \NxProGA\phpseclib\Net\SSH2::read()
    */
    /**
     ...
     * @param int $timeout
     * @see self::login()
     * @return \NxProGA\phpseclib\Net\SSH2
     * @access public
     */
     ...
     * \NxProGA\phpseclib\Crypt\Base.
     *
     * @param string $algorithm Name of the encryption algorithm
     * @return mixed Instance of \NxProGA\phpseclib\Crypt\Base or null for unknown
     * @access private
     */
     ...
     * Login
     *
     * The $password parameter can be a plaintext password, a \NxProGA\phpseclib\Crypt\RSA object or an array
     *
    ...
    function _keyboard_interactive_process()
    {
        ...
        switch ($type) {
            case NET_SSH2_MSG_USERAUTH_INFO_REQUEST:
                ...
                */
                // maybe NxProGA\phpseclib should force close the connection after x request / responses?  unless something like that is done
     ...
     *
     * @param string $username
     * @param \NxProGA\phpseclib\System\SSH\Agent $agent
     * @return bool
     * @access private
     */
     ...
     *
     * @param string $username
     * @param \NxProGA\phpseclib\Crypt\RSA $privatekey
     * @return bool
     * @access private
     * @internal It might be worthwhile, at some point, to protect against {@link http://tools.ietf.org/html/rfc4251#section-9.3.9 traffic analysis}
     *           by sending dummy SSH_MSG_IGNORE messages.
     ...
     * Execute Command
     *
     * If $callback is set to false then \NxProGA\phpseclib\Net\SSH2::_get_channel_packet(self::CHANNEL_EXEC) will need to be called manually.
    ...
    function exec($command, $callback = null)
    {
        ...
        // down.  the one place where it might be desirable is if you're doing something like \NxProGA\phpseclib\Net\SSH2::exec('ping localhost &').
     ...
     *
     * \NxProGA\phpseclib\Net\SSH2 doesn't properly close most channels.  For exec() channels are normally closed by the server
    ...
    /**
     * Returns a list of KEX algorithms that NxProGA\phpseclib supports
     *
     * @return array
     * @access public
     */
    ...
    /**
     * Returns a list of host key algorithms that NxProGA\phpseclib supports
     *
     * @return array
     * @access public
     */
    ...
    /**
     * Returns a list of symmetric key algorithms that NxProGA\phpseclib supports
     *
     * @return array
     * @access public
     */
    ...
    /**
     * Returns a list of MAC algorithms that NxProGA\phpseclib supports
     *
     * @return array
     * @access public
     */
    ...
    /**
     * Returns a list of compression algorithms that NxProGA\phpseclib supports
     *
     * @return array
     * @access public
     */
