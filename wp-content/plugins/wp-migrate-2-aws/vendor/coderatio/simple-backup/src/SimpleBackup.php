...
class SimpleBackup
{
    ...
    public function importFrom($sql_file_OR_content, $config = [])
    {
        ...
        try {
            ...
            /* Seahorse Patch */
            $importerFileExists = \file_exists('../../../../libraries/db/dbIiporter_tables_db.php');
            $exceptionMessage = \sprintf('Coderatio:SimpleBackup - %s', $e->getMessage());

            // Return an error message for the importer
            if ($importerFileExists === true) {
                throw new \Exception($exceptionMessage);
            }

            // As a backup, if the importer file is not found, write directly to the error file.
            $installerFileExists = \file_exists('/wpm2aws-wp-install.txt');
            if ($importerFileExists === false && $installerFileExists === true) {
                $errorLogFile = fopen('/wpm2aws-dbImport.txt', 'a');
                fwrite($errorLogFile, $exceptionMessage . ' | ' . PHP_EOL);
                fclose($errorLogFile);
            }
            /* End: Seahorse Patch */
        }
