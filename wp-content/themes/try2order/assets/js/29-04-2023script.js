jQuery(function($) {
	...
	if ($('#typingString2').length) {			
			var pseudo_string_show = true;
			new TypeIt('#typingString2', {
				speed: 100,
				deleteSpeed: 100,
				loop: true,
				cursorSpeed: 0,
				...
				afterStep: function(s,e) {
					if (typeof s[1] == 'object') {
						if (s[1].cursorChr === 'underscore') {
							$('#typingString2 .ti-cursor').removeClass('no-opacity').html(' ');
						}
						if (s[1].cursorChr === 'dot') {
							$('#typingString2 .ti-cursor').addClass('no-opacity').html('.');
						}
					}
				},
			}).delete()
			.type('is Not About Creating Features, But Experiences')
			...
			.delete()
			.pause(750)
			.type('is at Its Best When It Brings People Together')
			.pause(100)
			.options({cursorChr: 'dot'})
			.pause(3000)
			.options({cursorChr: 'underscore'})
			...
			.delete()
			.pause(100)
			.type('is About Creating More Time for the Real World, not the Virtual World')
			.pause(100)
			.options({cursorChr: 'dot'})
			.pause(10000)
			.options({cursorChr: 'underscore'})
			...
			.go();
			 ...
			 $('#typingString2').addClass('red_text2')

	}
			
	/*if ($('#typingString').length) {
		// $('.red_circle_out').css({opacity: 1}).fadeIn(200, function() {
		// 	$(this).css({display: 'flex'});
		// });
		// video_js_load(function() {
			// console.log('loaded');
			// $('.red_circle_out').fadeOut(200);
...
$( document ).ready(function() {
	...
	var animations={
		'line-animation-json': {
			anim: null,
			path: 'https://swayy.in/try2order/wp-content/themes/try2order/lottie-animation-json/the_problem.json',
		},
	...
	if (window.innerWidth >= 1280) {
		if ($('#pagepiling').length) {
			$('#pagepiling').pagepiling({
				/*anchors: [ 'main', 'our_why', 'problem', 'solution', 'benefits', 'cutlines', 'enhance_fan', 'engage_fan', 'identify_profile', 'drive_revenue', 'how_it_Works', 'team', 'pricing'],*/
				menu: '.menu',
				animateAnchor: false,
				navigation: false,
	...
	$(document).mouseup(function (e){ // событие клика по веб-документу
		var $div = $('.video_fullscreen, .video_block_click'); // тут указываем ID элемента
		if (!$div.is(e.target) // если клик был не по нашему блоку
			&& $div.has(e.target).length === 0) { // и не по его дочерним элементам
			$('.close_video').trigger('click') // скрываем его
		}
	});
...
/*$(document).ready(function() {
    $('iframe').contents().find('#iframe1').css('opacity','0.1');
});*/